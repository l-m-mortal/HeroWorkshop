...
Полный промпт для новой локальной сессии: `docs/PROMPT_local_handoff.md`.
...
8. **Текстуры пригорков (склонов)**: `python3 map_fix.py cliffs`. В карте два типа склонов,
   ...
   переводит все клетки склонов на берегу Radiant в CCgr, на берегу Dire в CCdi (сторона =
   берег реки: река = самая большая связная область воды, клетка сравнивается с ближайшей
   клеткой реки по диагонали; первая версия делила по диагонали карты и красила часть Dire
   у реки текстурой Radiant). Текстуры: `--radiant ФАЙЛ --dire ФАЙЛ` (png/blp,
   ...
   (в HQ-папке лежат `ReplaceableTextures/Cliff/Cliff0.blp` и `Cliff1.blp`, они и подхватились: Radiant склоны стали коричневыми, пользователь хочет позже подобрать текстуры из другого пака): `cliffs --scan` ищет CliffTypes.slk и текстуры склонов в HQ-папке,
