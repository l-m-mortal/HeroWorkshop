...
docked к правому краю экрана (между полосой очков и командной панелью), все
14 лавок показаны сразу двумя столбцами по 7 блоков — без вкладок, без
страниц. Каждый блок — имя лавки (ASCII) + сетка 4×3 иконок в тех же ячейках,
что у родной панели покупки (`Buttonpos` проданного юнита); имя и цена — в
тултипе.

В игре: команда в чат `-shop` и невидимая кнопка поверх штатной надписи
"SHOP" на командной панели открывают/закрывают окно (у каждого игрока — своё
состояние открыто/закрыто); клик по иконке товара — покупка: проверка
золота, списание и `UnitAddItemById` в инвентарь первого героя игрока
(упрощённый путь, без имитации штатной продажи лавкой и без выдачи через
курьера/фонтан — см. `docs/SHOP_UI_NOTES.md`). `map_fix.py shop-ui --undo
--apply` откатывает блок, не трогая `HW_COOLDOWN_*`.
