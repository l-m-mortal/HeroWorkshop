...
python3 workshop.py add-icon item:I0B4 icon.png       # своя PNG/BLP в библиотеку для ячейки (первая в списке вариантов)
...
python3 map_fix.py shop-ui [--undo] --apply           # окно магазина в стиле Dota 2 (колонка у правого края, без вкладок)
```
...
обычными кликовыми зданиями; Radiant/Dire-дубли с одинаковым именем лавки
сведены в одну) и внедряет в `war3map.j` один чистый JASS-блок (`shop_jass.py`,
план — `docs/SHOP_UI_PLAN.md`, вариант A), без FDF/TOC, без vJASS/Lua: панель
docked к правому краю экрана с небольшим отступом от края (между полосой
очков и командной панелью), все 14 лавок показаны сразу тремя столбцами по 5
блоков — без вкладок, без страниц. Каждый блок — имя лавки (ASCII) над своей
сеткой 4×3 иконок в тех же ячейках, что у родной панели покупки (`Buttonpos`
проданного юнита); имя и цена — в тултипе.

В игре: команда в чат `-shop` и полупрозрачная кнопка поверх штатной надписи
"SHOP" на командной панели открывают/закрывают окно (у каждого игрока — своё
состояние открыто/закрыто); клик по иконке товара — **настоящая покупка**:
`IssueNeutralImmediateOrderById` на живом юните-лавке (все 14 лавок карты
принадлежат `PLAYER_NEUTRAL_PASSIVE` — проверено по `war3map.j`), order id —
rawcode продаваемого юнита-обёртки (штатная конвенция `Sellunits`); списание
золота, создание предмета и попытка выдать его ближайшему союзному юниту —
делает сам движок Warcraft III, без имитации в JASS-коде (см.
`docs/SHOP_UI_NOTES.md` — там же про случай «герой не рядом с лавкой»).
`map_fix.py shop-ui --undo --apply` откатывает блок, не трогая
`HW_COOLDOWN_*`.

## Передача работы

Состояние проекта, открытые проблемы и команды: `docs/HANDOFF.md` (обновляется на каждом шаге).
